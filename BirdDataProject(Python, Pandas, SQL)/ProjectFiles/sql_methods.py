...
def highCounts(birdlist, year):
    ...
    if 1900 <= int(year) <= 2100:
        ...
        year  = ''
    for i in range(0, len(birdlist)):
        highCount = runCommand('SELECT Checklist, MAX(count) from "' + str(birdlist[i]) + '"' + year)
        if highCount[0][0]:
            countDate = runCommand("SELECT Month, Day, CHECKLISTS.Year, Hotspot from CHECKLISTS where ID = '" + highCount[0][0] + "';")
